 ...
 *    [function@GObject.type_class_ref] or through [function@GObject.type_create_instance]
 *    (this is being called by [function@GObject.object_new]) or through one of the above
 *    done on a type derived from @new_type_id.
 *
 * 3. This causes the type system to load the type's implementation by calling
 *    [function@GObject.type_plugin_use] and [method@GObject.TypePlugin.complete_type_info]
 *    on @new_type_plugin.
 *
 * 4. At some point the type's implementation isn't required anymore, e.g. after
 *    [function@GObject.type_class_unref] or [function@GObject.type_free_instance]
